...
async function saveExercise(){ const name=$("ex-name-input").value.trim(); if(!name){ showToast("Exercise name required"); return; } // If editing a builtin (string ID), generate a new numeric ID so Supabase accepts it
  const isBuiltin=editingExId&&String(editingExId).startsWith("builtin_");
  const id=(!editingExId||isBuiltin)?Date.now():editingExId; const ex={id,name,days:exEditDays,muscles:exEditMuscles,equipment:exEditEquip}; try{ await upsertCustomExercise(ex); const existing=customExercises.findIndex(e=>e.id===id); if(existing>=0) customExercises[existing]={...ex,isCustom:true}; else customExercises.push({...ex,isCustom:true}); mergeExercises(); $("ex-edit-form").classList.add("hidden"); editingExId=null; renderExerciseManagerList(); showToast(editingExId?"Exercise updated":"Exercise added"); } catch(e){ console.error(e); showToast("Error saving exercise"); } }
